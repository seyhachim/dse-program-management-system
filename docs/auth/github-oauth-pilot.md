...
3. Enable the GitHub provider and Supabase Auth **manual identity linking** on that project. Under Authentication → URL Configuration, allowlist the two precise preview URLs: `https://<preview-origin>/connect-github` and `https://<preview-origin>/github-sign-in?callback=1`. The OAuth callback does **not** contain a `next` query parameter or Telegram token; the app retains the return path only in browser same-tab session storage. Allowlist only trusted application origins, not a wildcard production domain.
4. Deploy the branch preview with `NEXT_PUBLIC_AUTH_MODE=supabase`, the preview project's normal public Supabase URL/key, and `NEXT_PUBLIC_GITHUB_PILOT_ENABLED=true`. Keep production's flag false until review and UAT pass. Never use a GitHub OAuth App secret as a `NEXT_PUBLIC_` variable.
...
- [ ] Confirm a password-login account with a different linked Supabase UID cannot be taken over by matching its email. Verify safe `next` and Telegram-linking return paths; make sure OAuth redirect URLs do not contain Telegram tokens. Verify logout, mobile experience, and no duplicate PMS Students/Users.
