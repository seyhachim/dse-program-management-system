...
PR #1139 must pass its own acceptance criteria and merge first. On #1141's exact final head verify `bun run typecheck`, `bun run lint`, `bun run test`, `bun run build`, Prisma validation and CI migration/security/authorization/API-contract jobs; inspect the root PR template and #1140 checkboxes. Isolated Supabase auto-link testing, recent reauthentication, adversarial integration coverage, one-student UAT, explicit rollback and owner-approved configuration remain required before this feature is considered release-ready. The production pilot flag remains off.
