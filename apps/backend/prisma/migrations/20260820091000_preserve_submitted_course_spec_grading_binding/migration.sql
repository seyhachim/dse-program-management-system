-- Issue #457 follow-up: once a Course Specification has entered review, its exact
-- grading-scale binding remains valid even if that programme policy is superseded
-- before the Course Specification itself is approved. The first submission still
-- requires the then-current Approved policy when the CourseSpec has no effective
-- date, but later review transitions must not deadlock or silently rebind history.
...
BEGIN
  ...
  -- On the first transition into review, an undated CourseSpec must use the
  -- current Approved programme policy. After that point, a later supersession
  -- does not invalidate the exact version already pinned at submission.
  IF entering_review AND NEW."effectiveFrom" IS NULL AND scale_status <> 'Approved' THEN
    RAISE EXCEPTION 'An undated CourseSpec must use the current Approved grading scale when submitted';
  END IF;

  -- Dated CourseSpecs may legitimately bind a Superseded version as long as
  -- their effective date falls inside that version's historical interval.
