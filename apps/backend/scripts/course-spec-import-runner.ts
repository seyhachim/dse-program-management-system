...
async function createRuntimeImporter(): Promise<string> {
  ...
  const prismaMarker = "const prisma = new PrismaClient();";
  if (!source.includes(prismaMarker)) {
    throw new Error("Could not configure importer transaction timeout: PrismaClient constructor not found");
  }
  ...
  const schemaMarker = "            mappedPlos: z.array(z.string()).default([]),";
  if (!source.includes(schemaMarker)) {
    throw new Error("Could not add canonical CLO SLT support: CLO schema marker not found");
  }
  source = source.replace(
    schemaMarker,
    `${schemaMarker}\n            sltHours: z.number().nullable().optional(),`,
  );

  const writeMarker = "      sltHours: null,";
  if (!source.includes(writeMarker)) {
    throw new Error("Could not add CLO SLT persistence: importer write marker not found");
  }
  source = source.replace(writeMarker, "      sltHours: clo.sltHours ?? null,");
