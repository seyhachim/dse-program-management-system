...
function activeLearningFromRaw(raw: string): string[] {
  return [
    ...new Set(
      ACTIVE_LEARNING_ALIASES.filter(([rx]) => rx.test(raw)).map(
        ([, id]) => id,
      ),
    ),
  ];
...
function assessmentType(
  ...
  if (v.includes("exam") || v.includes("midterm") || v.includes("mid-term"))
    return "Exam";
...
function prismaCourseType(
  value?: string | null,
): "Basic" | "Core" | "Elective" | "Specialization" | "MoeysHeip" | null {
...
function grandTotalSlt(doc: CanonicalCourse): number | null {
  for (const row of doc.studentLearningTime?.continuousAssessmentRows ?? []) {
    if (cleanText(row[0]).toLowerCase() === "grand total slt") {
      const n = Number(
        row.findLast((cell) => /^\d+(\.\d+)?$/.test(cleanText(cell))),
      );
...
function parsePolicy(raw?: string | null) {
  ...
  const labels: Array<[keyof ReturnType<typeof emptyPolicy>, RegExp]> = [
    ...
    [
      ...
      /Homework\s*&?\s*Assignments|Assignments?\s*&?\s*Late Submission/i,
    ],
    ["examinationRules", /Examinations?/i],
    ["penaltiesConsequences", /Penalties|Consequences/i],
  ];
  const found = labels
    .map(([key, rx]) => ({ key, match: rx.exec(text) }))
    .filter((x) => x.match)
...
async function resolveLecturer(
  doc: CanonicalCourse,
  ...
  if (!target)
    return { id: null, warning: "No primary lecturer name/email in document" };
  ...
  const matches = users.filter(
    (u) =>
      u.roleAssignments.some((r) => r.role.slug === "lecturer") &&
...
async function ensureTeachingMethod(tx: Tx, name: string) {
  return tx.teachingMethod.upsert({
    where: { name },
    create: { name },
    update: {},
    select: { id: true },
...
async function ensureAssessmentMethod(tx: Tx, name: string) {
  return tx.assessmentMethod.upsert({
    where: { name },
    create: { name },
    update: {},
    select: { id: true },
...
async function importOne(
  ...
  if (errors.length)
    return {
      ...
      action: "blocked",
      warnings,
      error: `Extraction errors: ${errors.join("; ")}`,
    };
  if (warnings.length && !options.allowWarnings)
    return {
      file,
      courseCode,
      action: "blocked",
      warnings,
      error: "Extraction warnings require --allow-warnings after manual review",
    };

  const lecturer = await resolveLecturer(doc);
  if (lecturer.warning) warnings.push(lecturer.warning);
  if ((doc.lecturers.coLecturers ?? []).length)
    warnings.push(
      "Co-lecturers were preserved in source JSON but not imported because they belong to a term-specific Offering",
    );
  if (doc.course.availability?.semester || doc.course.availability?.year)
    warnings.push(
      "Semester/programme year were not imported because the Offering model requires a real term",
    );
  ...
  if (existingCourse?.spec && !options.replaceExisting)
    return {
      ...
      warnings: [
        ...warnings,
        "Existing CourseSpec found; pass --replace-existing to replace it",
      ],
    };
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (assessmentRows.length)
      await tx.courseSpecAssessmentItem.createMany({ data: assessmentRows });
