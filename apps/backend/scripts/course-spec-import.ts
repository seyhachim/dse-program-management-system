...
const CanonicalCourseSchema = z.object({
  ...
  course: z.object({
    ...
    credits: z.object({ total: z.number().nullable().optional() }).passthrough().optional(),
    prerequisites: z.string().nullable().optional(),
    courseType: z.string().nullable().optional(),
    availability: z
      .object({ semester: z.number().nullable().optional(), year: z.number().nullable().optional() })
      .passthrough()
      .optional(),
  ...
  validation: ValidationSchema.default({ status: "valid", warnings: [], errors: [] }),
}).passthrough();
...
function normalizeName(value: string): string {
  return cleanText(value).toLowerCase().replace(/\b(prof|dr|mr|mrs|ms)\.?\b/g, "").replace(/[^a-z0-9]+/g, " ").trim();
}
...
async function resolveLecturer(doc: CanonicalCourse): Promise<{ id: string | null; warning?: string }> {
  ...
  const users = await prisma.user.findMany({ select: { id: true, name: true, roleAssignments: { select: { role: { select: { slug: true } } } } } });
  const matches = users.filter((u) => u.roleAssignments.some((r) => r.role.slug === "lecturer") && normalizeName(u.name) === target);
  if (matches.length === 1) return { id: matches[0]!.id };
  return { id: null, warning: matches.length > 1 ? `Ambiguous lecturer match for ${doc.lecturers.primary.name}` : `Lecturer not found: ${doc.lecturers.primary.name}${email ? ` <${email}>` : ""}` };
}
...
function dueWeekForAssessment(doc: CanonicalCourse, name: string): number | null {
  ...
  for (const week of doc.lessonPlan?.weeks ?? []) {
    ...
    if (hay.includes(needle) || needle.includes(hay.replace(/^\d+\s*/, "").trim())) return week.week;
  }
  return null;
}
...
async function importOne(doc: CanonicalCourse, file: string, options: CliOptions): Promise<ImportResult> {
  ...
  const existingCourse = await prisma.course.findUnique({ where: { code: courseCode }, include: { spec: { select: { id: true } } } });
  if (existingCourse?.spec && !options.replaceExisting) return { file, courseCode, action: "skipped", warnings: [...warnings, "Existing CourseSpec found; pass --replace-existing to replace it"] };
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const oldSpec = await tx.courseSpec.findUnique({ where: { courseId: course.id }, select: { id: true } });
    if (oldSpec && options.replaceExisting) await tx.courseSpec.delete({ where: { courseId: course.id } });
    ...
    const cloRows = doc.clos.map((clo, order) => ({
      id: stableId(courseCode, "clo", order), courseSpecId: spec.id, order,
      description: clo.description, level: clo.level ?? null, mappedPlos: clo.mappedPlos,
      sltHours: null, status: clo.status === "inactive" ? ("Inactive" as const) : ("Active" as const),
      notes: "", activeLearningStrategyIds: activeStrategyIds,
    }));
    ...
    const cloTeachingLinks = doc.clos.flatMap((clo) => aliasesFromRaw(clo.teachingMethodsRaw ?? "", TEACHING_METHOD_ALIASES).flatMap((name) => {
      const id = teachingMethodIds.get(name); const cloId = cloIdByCode.get(clo.code); return id && cloId ? [{ courseSpecId: spec.id, cloId, teachingMethodId: id }] : [];
    }));
    ...
    const cloAssessmentLinks = doc.clos.flatMap((clo) => aliasesFromRaw(clo.assessmentMethodsRaw ?? "", ASSESSMENT_METHOD_ALIASES).flatMap((name) => {
      const id = assessmentMethodIds.get(name); const cloId = cloIdByCode.get(clo.code); return id && cloId ? [{ courseSpecId: spec.id, cloId, assessmentMethodId: id }] : [];
    }));
    ...
    const weekRows = (doc.lessonPlan?.weeks ?? []).map((week, order) => {
      ...
      return {
        id: stableId(courseCode, "week", week.week), courseSpecId: spec.id, order, week: week.week,
        topic: cleanText(week.lectureTopic), cloCodes: week.clos, lloItems: llos.map((x) => x.description),
        ...
        lectureHours, tutorialHours, practiceHours, otherHours, selfStudyHours: selfStudy.get(week.week) ?? null,
    ...
    const assessmentRows = doc.assessments.map((item, order) => ({
      id: stableId(courseCode, "assessment", order), courseSpecId: spec.id, order, name: item.name,
      type: assessmentType(item.name), description: "", mode: item.mode.toLowerCase().includes("group") ? ("Group" as const) : ("Individual" as const),
      status: "Active" as const, cloCodes: item.clo ? [item.clo] : [], feedbackMethod: "", feedbackTimeline: "",
      weight: item.weightPercent ?? null, dueWeek: dueWeekForAssessment(doc, item.name), durationWeeks: null,
      format: "", submissionMethod: "", instructions: "", rubric: "", mappedPlos: item.plos, notes: "",
    }));
    ...
    for (const week of doc.lessonPlan?.weeks ?? []) {
      for (const title of week.resourcesRaw.split(/[,;]+/).map(cleanText).filter(Boolean)) {
        ...
        if (existing != null) {
          ...
          row.evidenceWeekIds = [...new Set([...(row.evidenceWeekIds ?? []), stableId(courseCode, "week", week.week)])];
        } else {
          seenResources.set(key, resourceRows.length);
          resourceRows.push({ id: stableId(courseCode, "resource", resourceRows.length), courseSpecId: spec.id, order: resourceRows.length, resourceType: "Weekly resource", title, url: "", notes: "", evidenceWeekIds: [stableId(courseCode, "week", week.week)], kind: "OTHER", authors: "", publisher: "", year: "", isbn: "", basedOn: "" });
        }
      }
    }
    ...
    for (const [kind, list] of [["REQUIRED", refs?.required ?? []], ["RECOMMENDED", refs?.recommended ?? []]] as const) {
      for (const ref of list) {
        const title = cleanText(String(ref.title ?? "")); if (!title) continue;
        resourceRows.push({ id: stableId(courseCode, "reference", resourceRows.length), courseSpecId: spec.id, order: resourceRows.length, resourceType: "Reference", title, url: "", notes: "Imported from legacy DOCX reference table", evidenceWeekIds: [], kind, authors: cleanText(String(ref.authors ?? "")), publisher: cleanText(String(ref.publisher ?? "")), year: cleanText(String(ref.year ?? "")), isbn: cleanText(String(ref.isbn ?? "")), basedOn: "" });
      }
    }
    ...
    if (responsibilities.length) await tx.courseSpecStudentResponsibility.createMany({ data: responsibilities.map((text, order) => ({ id: stableId(courseCode, "responsibility", order), courseSpecId: spec.id, order, text })) });
    ...
    const resourceTypes = [...new Set((doc.lessonPlan?.weeks ?? []).flatMap((w) => {
      const raw = w.resourcesRaw.toLowerCase(); const values: string[] = [];
      if (raw.includes("slide")) values.push("Slides"); if (raw.includes("dataset") || raw.includes("data set")) values.push("Datasets");
      if (raw.includes("worksheet")) values.push("Worksheets"); if (raw.includes("video")) values.push("Videos"); if (raw.includes("case")) values.push("Case Studies"); return values;
    }))];
    const technologyTypes = [...new Set((doc.lessonPlan?.weeks ?? []).flatMap((w) => {
      const raw = w.resourcesRaw.toLowerCase(); const values: string[] = [];
      if (raw.includes("jupyter")) values.push("Jupyter"); if (raw.includes("github")) values.push("GitHub"); if (raw.includes("colab")) values.push("Google Colab"); if (raw.includes("lms")) values.push("LMS"); return values;
    }))];
    ...
    const sections: Array<{ key: string; complete: boolean }> = [
      { key: "courseInfo", complete: true }, { key: "clos", complete: cloRows.length > 0 },
      { key: "assessmentPlan", complete: assessmentRows.length > 0 }, { key: "slt", complete: weekRows.length > 0 },
      { key: "resources", complete: resourceRows.length > 0 }, { key: "responsibility", complete: responsibilities.length > 0 },
      { key: "policy", complete: Object.values(policy).some(Boolean) }, { key: "teachingLearning", complete: false },
    ];
    await tx.courseSpecSection.createMany({ data: sections.filter((s) => s.complete || s.key === "teachingLearning").map((s) => ({ courseSpecId: spec.id, sectionKey: s.key, status: s.complete ? ("Complete" as const) : ("Draft" as const) })) });
  });
...
async function main() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      results.push({ file, courseCode: "UNKNOWN", action: "failed", warnings: [], error: error instanceof Error ? error.message : String(error) });
    }
  }
  const summary = {
