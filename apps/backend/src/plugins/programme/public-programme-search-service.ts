...
import {
  ...
  type SearchablePublicProgrammeFaq,
...
function canonicalToken(token: string): string {
  ...
  if (/^[a-z]+$/.test(token) && token.length > 4 && token.endsWith("s"))
    return token.slice(0, -1);
  return token;
}
function tokens(value: string): string[] {
  const result = normalizePublicSearchText(value)
    .split(" ")
    .filter(Boolean)
    .map(canonicalToken)
...
function scoreFaq(question: string, faq: RankableFaq): number {
  ...
  const answerTokens = new Set(
    tokens(`${faq.shortAnswer ?? ""} ${faq.answer}`),
  );
  const keywordTokens = new Set(tokens((faq.keywords ?? []).join(" ")));
  const categoryTokens = new Set(
    (CATEGORY_TOKENS[faq.category] ?? []).map(canonicalToken),
  );
...
export function rankPublishedFaqs(
  ...
  return faqs
    ...
    .sort(
      (a, b) =>
        b.score - a.score || a.faq.question.localeCompare(b.faq.question),
    );
}
...
export const publicProgrammeSearchService = {
  ...
  ): Promise<PublicAskDseResult> {
    const faqs: SearchablePublicProgrammeFaq[] =
      await publicProgrammeReadService.listFaqsForSearch(programmeId, locale);
