...
function read(versionId: string, courses: CurriculumCourse[], id = curriculumId): ProgrammeCurriculumRead {
  ...
  return {
    curriculum: { id, programmeId: "dse", code: "DSE", name: "DSE" },
    selectedVersion: {
      id: versionId,
      versionMajor: 1,
      versionMinor: 0,
...
describe("curriculum comparison", () => {
  ...
  test("rejects comparison across different curricula", () => {
    expect(() => compareCurriculumReads(
      read(crypto.randomUUID(), [], crypto.randomUUID()),
      read(crypto.randomUUID(), [], crypto.randomUUID()),
    )).toThrow("Curriculum versions must belong to the same curriculum");
  });
});
