...
export class FixedWindowRateLimiter implements RateLimiter {
  ...
  check(key: string, limit: number, windowMs: number, nowMs = Date.now()): RateLimitDecision {
    this.checks += 1;
    if (this.checks % SWEEP_INTERVAL === 0 || this.buckets.size >= MAX_BUCKETS) this.sweep(nowMs);
    ...
    if (current.count >= limit) return { allowed: false, retryAfterSeconds, remaining: 0 };
...
export function getPublicAbuseProtectionConfig(env: NodeJS.ProcessEnv = process.env): PublicAbuseProtectionConfig {
  return {
...
export function purposeHmac(secret: string, purpose: string, value: string | number): string {
  return createHmac("sha256", secret).update(`${purpose}\0${String(value)}`).digest("base64url");
}
