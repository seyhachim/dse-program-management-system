...
export function RollCallDialog(props: RollCallDialogProps) {
  ...
  useEffect(() => {
    if (confirmedContext && confirmedContext !== contextKey) {
      setConfirmedContext(null);
    }
  }, [confirmedContext, contextKey]);

  if (!open) return null;
  if (confirmedContext === contextKey) {
    return <ActiveRollCallDialog {...props} />;
  }
        ...
        <div className="overflow-hidden rounded-lg">
          <div className="border-b border-border bg-primary/5 px-6 py-5">
            <DialogTitle className="text-xl font-semibold text-foreground">
              Confirm class before marking attendance
            </DialogTitle>
            <DialogDescription className="mt-1 text-sm text-muted-foreground">
              Check the course and class carefully. Roll Call shortcuts stay disabled until you confirm.
            </DialogDescription>
          </div>
          ...
          <div className="space-y-5 px-6 py-6">
            <div className="rounded-xl border-2 border-primary/30 bg-primary/5 p-5 text-center">
              ...
              <div className="mt-3 flex flex-wrap justify-center gap-2">
                ...
                <span className="rounded-full border border-primary/30 bg-background px-3 py-1 text-sm font-semibold text-foreground">
                  Class {sectionCode}
                </span>
              </div>
            </div>
            ...
            <dl className="grid grid-cols-3 gap-3 text-center">
              <div className="rounded-lg border border-border bg-muted/30 p-3">
                <dt className="text-xs text-muted-foreground">Teaching week</dt>
                <dd className="mt-1 font-semibold text-foreground">{weekLabel}</dd>
              </div>
              <div className="rounded-lg border border-border bg-muted/30 p-3">
                <dt className="text-xs text-muted-foreground">Attendance date</dt>
                <dd className="mt-1 font-semibold text-foreground">{formatAttendanceDate(date)}</dd>
              </div>
              <div className="rounded-lg border border-border bg-muted/30 p-3">
                <dt className="text-xs text-muted-foreground">Students</dt>
                <dd className="mt-1 font-semibold tabular-nums text-foreground">{records.length}</dd>
              </div>
            </dl>
            ...
            <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button
                ...
                className="h-11 rounded-md border border-border bg-background px-4 text-sm font-semibold text-foreground hover:bg-muted"
              >
                Cancel
              </button>
              <button
                type="button"
                disabled={!offering || records.length === 0}
                onClick={() => setConfirmedContext(contextKey)}
                className="h-11 rounded-md bg-primary px-5 text-sm font-semibold text-primary-foreground hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Start {courseCode} {sectionCode} Roll Call
              </button>
            </div>
          </div>
        </div>
