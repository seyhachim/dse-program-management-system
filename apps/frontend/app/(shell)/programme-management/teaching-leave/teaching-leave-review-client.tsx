          ...
          {requests.map((request) => (
            <article key={request.id} className="rounded-xl border border-border bg-card p-5 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap justify-end gap-2">
                <button disabled={busyId === request.id} onClick={() => void decide(request, "REQUEST_CHANGES")} className="rounded-md border border-input px-3 py-2 text-sm">Request changes</button>
                <button disabled={busyId === request.id} onClick={() => void decide(request, "REJECT")} className="rounded-md border border-destructive/40 px-3 py-2 text-sm text-destructive">Reject</button>
                <button disabled={busyId === request.id} onClick={() => void decide(request, "APPROVE")} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground">{busyId === request.id ? "Saving…" : "Approve"}</button>
              </div>
            </article>
          ))}
        </div>
