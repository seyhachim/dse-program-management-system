...
function AchievementIcon({ badge }: { badge: PortalCourseAchievementBadge }) {
  if (!badge.achieved) return <Lock className="h-3.5 w-3.5" aria-hidden="true" />;

  switch (badge.kind) {
    case "great_start":
      return <Leaf className="h-3.5 w-3.5" aria-hidden="true" />;
    case "reliable_learner":
      return <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />;
    case "perfect_attendance":
      return <CalendarCheck2 className="h-3.5 w-3.5" aria-hidden="true" />;
    case "strong_performance":
      return <Target className="h-3.5 w-3.5" aria-hidden="true" />;
    case "course_excellence":
      return <Trophy className="h-3.5 w-3.5" aria-hidden="true" />;
  }
}

function MonitorRoleChip({ role }: { role: ClassResponsibilityRole }) {
  const isClassMonitor = role === "ClassMonitor";
  const Icon = isClassMonitor ? Crown : ShieldCheck;
  const label = isClassMonitor ? "Class Monitor" : "Sub-class Monitor";

  return (
    <span
      className="inline-flex items-center gap-1 rounded-full bg-primary/12 px-2.5 py-1 text-[10px] font-semibold text-primary ring-1 ring-primary/30 sm:text-[11px]"
      aria-label={`Course role: ${label}`}
    >
...
export function CourseAchievementBadges({
  ...
  if (!summary && !monitorRole) return null;

  return (
    <div className="mt-3 space-y-2" aria-label="Course achievements and responsibilities">
      {monitorRole ? (
        <div className="flex flex-wrap gap-1.5">
          <MonitorRoleChip role={monitorRole} />
        </div>
      ) : null}

      {summary ? (
        <div className="flex flex-wrap gap-1.5" aria-label="Course achievement badges">
          {summary.badges.map((badge) => (
            <span
              key={badge.kind}
              className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-semibold leading-none ring-1 sm:text-[11px] ${
