...
function PeriodCard({ period }: { period: Period }) {
  ...
  return (
    <section
      className={`rounded-2xl border bg-card p-4 transition md:p-5 ${
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="mt-1 break-words text-lg font-semibold">
            {academicSemesterLabel(period.semester)}
          </h3>
        </div>
        <StateBadge state={state} />
      </div>

      <div className="mt-4 rounded-xl bg-muted/35 p-3 md:mt-5 md:p-4">
        <div className="flex min-w-0 items-start gap-2 text-sm font-medium">
          <CalendarDays className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
          <span className="break-words">
            {formatAcademicDate(period.teachingStart)} –{" "}
            {formatAcademicDate(period.teachingEnd)}
          </span>
        </div>
        {state === "current" ? (
          <div className="mt-3">
      ...
      <dl className="mt-4 grid gap-3 text-sm">
        <div className={MOBILE_STUDENT_PORTAL_LAYOUT.periodFact}>
        ...
        <div className={MOBILE_STUDENT_PORTAL_LAYOUT.periodFactLast}>
...
function EventTimeline({
    ...
    return (
      <div className="rounded-xl border border-dashed border-border px-4 py-8 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Semester teaching dates above remain your official calendar reference.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={MOBILE_STUDENT_PORTAL_LAYOUT.timeline}>
      {events.map((event) => {
        ...
        return (
          <div
            key={event.id}
            className={MOBILE_STUDENT_PORTAL_LAYOUT.timelineRow}
          >
            <div className={MOBILE_STUDENT_PORTAL_LAYOUT.timelineDate}>
              <p className="text-xs font-semibold tabular-nums text-foreground">
                {formatAcademicDate(event.startDate)}
              </p>
              {event.endDate ? (
                <p className="mt-0.5 text-[11px] text-muted-foreground">
                  to {formatAcademicDate(event.endDate)}
                </p>
              ) : null}
            </div>
            <div
              className={`relative min-w-0 rounded-xl border p-3.5 ${
                isNext
            ...
            >
              <span
                aria-hidden="true"
                className={`${MOBILE_STUDENT_PORTAL_LAYOUT.timelineDot} ${
                  state === "completed"
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 break-words text-xs text-muted-foreground">
                    {event.type}
                    {event.semester
                      ? ` · ${academicSemesterLabel(event.semester)}`
                      : ""}
                  </p>
                  {event.note ? (
...
export function PortalAcademicCalendar() {
  ...
  if (error || !data) {
    return (
      <PortalError message={error ?? "Could not load your Academic Calendar"} />
    );
  }
  ...
  if (data.status === "unavailable") {
    return (
      <div className="mx-auto max-w-4xl">
        <section className="rounded-2xl border border-dashed border-border bg-card p-6 text-center md:p-12">
          ...
          <div className="mx-auto mt-5 flex w-fit max-w-full items-center gap-2 rounded-full border border-border bg-muted/40 px-3 py-1.5 text-xs text-muted-foreground">
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 md:space-y-6">
      <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <div className="border-b border-border bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-4 md:p-7">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between lg:gap-5">
            <div className="min-w-0">
              ...
              <h2 className="mt-0.5 break-words text-2xl font-bold tracking-tight sm:text-3xl">
                {data.academicYear.label}
              </h2>
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                These dates are matched automatically to your programme
                progression. You do not need to choose a year manually.
              </p>
            </div>
              ...
              {data.nextEvent ? (
                <>
                  ...
                  <p className="mt-1 break-words text-sm text-muted-foreground">
                    ...
                    {data.nextEvent.endDate
                      ? ` – ${formatAcademicDate(data.nextEvent.endDate)}`
                      : ""}
                  </p>
                </>
              ) : (
                <p className="mt-2 text-sm text-muted-foreground">
                  No upcoming academic event is currently published.
                </p>
              )}
            </div>
          </div>
        ...
        <div className="grid gap-3 p-4 text-xs text-muted-foreground sm:grid-cols-3 md:px-7">
          <span className="flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 shrink-0 text-primary" /> Published
            dates only
          </span>
          <span className="flex items-center gap-2">
            <CalendarCheck2 className="h-4 w-4 shrink-0 text-primary" />
            Auto-matched to your study year
          </span>
          <span className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 shrink-0 text-primary" /> No
            cross-year fallback
          </span>
        </div>
      </section>
      ...
      <section>
        <div className="mb-3">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Teaching, examination, and break periods at a glance.
          </p>
        </div>
      ...
      <section className="rounded-2xl border border-border bg-card p-4 md:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Registration, holidays, exams, and other published programme
              events.
            </p>
          </div>
