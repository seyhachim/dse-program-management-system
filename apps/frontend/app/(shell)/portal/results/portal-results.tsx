...
export function PortalResults() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4">
      {data.map((course) => {
        const hiddenProvisionalCount = course.provisionalResultAccess?.hiddenProvisionalAssessmentCount ?? 0;
        return (
          <Link
          ...
          >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {course.courseGradeComplete
                        ? "Complete"
                        : `${course.completedGradeWeight}% of ${course.configuredGradeWeight}% grade weight visible`}
                    </p>
                  </div>
                </div>
                <div className="flex min-w-48 items-center gap-3 rounded-xl border border-border px-3 py-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">Based only on visible mapped evidence</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {hiddenProvisionalCount > 0 ? (
              <div className="mt-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-amber-900 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200">
                <LockKeyhole className="mt-0.5 h-5 w-5 shrink-0" />
                <div>
                  <p className="font-semibold">Final course survey required for provisional results</p>
                  <p className="mt-1 text-sm">
                    {hiddenProvisionalCount} provisional assessment result{hiddenProvisionalCount === 1 ? " is" : "s are"} temporarily hidden. Finalized results remain visible and unchanged.
                  </p>
                  <p className="mt-2 text-xs font-medium">Open course to complete the anonymous survey →</p>
                </div>
              </div>
            ) : null}
              ...
              {course.achievements.map((item) => (
                <div key={item.code} className="rounded-xl bg-muted/40 p-3">
                  ...
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    {item.evidenceCount} visible mapped assessment result{item.evidenceCount === 1 ? "" : "s"}
                  </p>
                </div>
              ))}
            </div>
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Course grade and CLO achievement are separate calculations. {course.assessments.filter((item) => item.result).length} of {course.assessments.length} assessment results are currently visible.
            </p>
          </Link>
        );
      })}
