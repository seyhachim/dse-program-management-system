...
function courseInformationTable(info: CourseDocumentModel["courseInformation"]) {
  ...
  const labelValueRow = (label: string, value: string) => row(
    cell(label, { bold: true, shade: LABEL, width: w[0] }),
    cell(value, { width: w[1]! + w[2]! + w[3]!, columnSpan: 3 }),
  );
  const fourCellRow = (label1: string, value1: string, label2: string, value2: string) => row(
...
function mappingTable(document: CourseDocumentModel) {
  ...
  const rows = [new TableRow({ children: [headerCell("CLO", w[0]), headerCell("PLO", w[1]), headerCell("C/A/P Level", w[2]), headerCell("Teaching Method", w[3]), headerCell("Assessment Methods", w[4])] })];
  for (const row of document.mapping) {
    const rowValues = [row.cloCode, values(row.ploCodes), row.level, values(row.teachingMethods), values(row.assessmentMethods)];
...
function cloPloMatrixTable(document: CourseDocumentModel, mode: "percent" | "hours") {
  ...
  const rows = [new TableRow({ children: [headerCell("CLO", w[0]), ...PLOS.map((plo, i) => headerCell(plo.id, w[i + 1]))] })];
  for (const row of document.mapping) {
    rows.push(new TableRow({ children: [
      ...
      ...PLOS.map((plo, i) => {
        ...
        if (mode === "percent") return cell(row.focusCode && row.focusPercent != null ? `${row.focusCode} (${row.focusPercent}%)` : "—", { width });
...
function sltTable(document: CourseDocumentModel) {
  ...
  const sumOf = (key: keyof CourseDocumentModel["weeklyPlan"][number]) => String(document.weeklyPlan.reduce((s, week) => s + (Number(week[key]) || 0), 0));
  for (const week of document.weeklyPlan) {
    const rowValues = [week.week, week.topic, values(week.cloCodes), week.lectureHours, week.tutorialHours, week.practiceHours, week.otherHours, week.selfStudyHours, week.sltHours ? `${week.sltHours} h` : "—"];
    rows.push(new TableRow({ children: rowValues.map((v, i) => cell(v, { width: w[i] })) }));
  }
  rows.push(new TableRow({ children: [
...
function rubricCell(assessment: CourseDocumentModel["assessments"][number], width?: number) {
  ...
  const href = typeof window !== "undefined" ? new URL(assessment.rubricUrl, window.location.origin).toString() : assessment.rubricUrl;
  return new TableCell({
    width: width ? { size: width, type: WidthType.DXA } : undefined,
    verticalAlign: VerticalAlign.CENTER,
    margins: { top: 55, bottom: 55, left: 70, right: 70 },
    children: [new Paragraph({ spacing: { before: 0, after: 40, line: 220 }, children: [new ExternalHyperlink({ link: href, children: [new TextRun({ text: `${assessment.rubricName} ↗`, font: FONT, size: SMALL, color: COURSE_DOCUMENT_STYLE.colors.link.replace("#", ""), underline: { type: "single" } })] })] })],
  });
}

function assessmentTable(document: CourseDocumentModel) {
  const w = colWidths([7, 8, 10, 23, 7, 7, 24, 14]);
  const headers = ["CLOs", "PLO", "C/A/P Level", "Assessment & Description", "G/I", "Weight (%)", "Evaluation Definition", "Rubric"];
  const rows = [new TableRow({ children: headers.map((h, i) => headerCell(h, w[i])) })];
  for (const assessment of document.assessments) {
    const assessmentDescription = assessment.description ? `${assessment.name}\n${assessment.description}` : assessment.name;
    rows.push(new TableRow({ children: [
      cell(values(assessment.cloCodes), { width: w[0] }), cell(values(assessment.mappedPlos), { width: w[1] }), cell(values(assessment.capLevels), { width: w[2] }), cell(assessmentDescription, { width: w[3] }), cell(assessment.mode === "group" ? "G" : "I", { width: w[4] }), cell(assessment.weight ? `${assessment.weight}%` : "—", { width: w[5] }), cell(assessment.evaluationDefinition, { width: w[6] }), rubricCell(assessment, w[7]),
    ] }));
  }
  rows.push(new TableRow({ children: [cell("Total", { bold: true, width: w[0] }), cell("", { width: w[1] }), cell("", { width: w[2] }), cell("", { width: w[3] }), cell("", { width: w[4] }), cell(`${document.totals.assessmentWeight}%`, { bold: true, width: w[5] }), cell("", { width: w[6] }), cell("", { width: w[7] })] }));
  return table(rows, w);
}

function lessonPlanTable(weeks: CourseDocumentModel["weeklyPlan"]) {
  const w = colWidths([5, 9, 15, 8, 20, 18, 15, 10]);
  const headers = ["Week", "Hour (L/T/P/O)", "Topic", "CLO", "Lesson Learning Outcomes", "Teaching Method / Activity", "Assessment", "Resources"];
  const rows = [new TableRow({ children: headers.map((h, i) => headerCell(h, w[i])) })];
  for (const week of weeks) {
    const rowValues = [week.week, [week.lectureHours, week.tutorialHours, week.practiceHours, week.otherHours].map((h) => h || "0").join("/"), week.topic, values(week.cloCodes), week.lloItems.length ? week.lloItems.map((v, i) => `LLO${i + 1}: ${v}`).join("\n") : "—", values(week.teachingMethods.length ? week.teachingMethods : week.learningActivities), values([week.assessment, ...week.assessmentMethods].filter(Boolean)), values(week.resources)];
    rows.push(new TableRow({ children: rowValues.map((v, i) => cell(v, { width: w[i] })) }));
  }
  return table(rows, w);
}
...
function bulletedList(items: string[]) {
  return items.map((item) => new Paragraph({ bullet: { level: 0 }, spacing: { before: 0, after: 60, line: 220 }, children: [text(item, false, SMALL)] }));
}

function policyParagraphs(policy: CourseDocumentModel["policy"]) {
  const sections: [string, string][] = [["Attendance & Preparation", policy.attendancePreparation], ["Academic Integrity", policy.academicIntegrity], ["Homework & Assignments", policy.assignmentsLateSubmission], ["Examinations", policy.examinationRules], ["Penalties", policy.penaltiesConsequences]];
  return sections.flatMap(([label, value]) => [paragraph(label, true, SMALL), paragraph(value || "—", false, SMALL)]);
}
...
export async function exportCourseSpecWord(document: CourseDocumentModel) {
  ...
  children.push(paragraph("Programme Learning Outcomes — Percentages", true, SMALL));
  children.push(cloPloMatrixTable(document, "percent"));
  children.push(paragraph("Fully (F) indicates a focus of more than 50% of the total SLT on this PLO, Moderate (M) indicates a focus of 31%–50% of the total SLT, and Partial (P) indicates a focus of less than 30% of the total SLT on the PLO.", false, SMALL));
  children.push(paragraph("Programme Learning Outcomes — Total Hours for Student Learning Time (SLT)", true, SMALL));
  ...
  if (document.resources.length === 0) children.push(paragraph("No required resources have been confirmed.", false, SMALL));
  else children.push(resourcesTable(document.resources));

  children.push(new Paragraph({ children: [new PageBreak()] }));
  children.push(sectionTitle("21", "Student Responsibility"));
  if (document.responsibilities.length === 0) children.push(paragraph("No student responsibilities have been recorded.", false, SMALL));
  else children.push(...bulletedList(document.responsibilities));
