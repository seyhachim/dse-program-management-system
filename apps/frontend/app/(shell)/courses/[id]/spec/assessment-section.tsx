...
export function AssessmentSection({
  ...
  /*
   * assessmentTotalWeight() remains the source of truth for the existing
  ...
  const assessedCloCodes = useMemo(() => {
    ...
    for (const assessment of activeAssessments) {
      for (const code of assessment.cloCodes) {
        codes.add(code);
      }
    }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Assessment health / alignment summary */}
          ...
          <div className="mt-4">
            <span
              className={`text-2xl font-bold ${
                totalOk ? "text-foreground" : "text-status-live"
              }`}
            >
              ...
              <tbody>
                {value.map((assessment, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 pr-3">
                      <span
                        className={`inline-flex rounded-md px-2 py-0.5 text-xs font-medium ${assessmentTypeChip(
                          assessment.type,
                        )}`}
                      >
                        {assessment.type}
          ...
          <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              ...
              {totalOk
                ...
                : `Total weight is ${
                    Math.round(total * 100) / 100
                  }% — active assessments must sum to 100%.`}
            </div>
          </div>
        ) : null}
      </section>
