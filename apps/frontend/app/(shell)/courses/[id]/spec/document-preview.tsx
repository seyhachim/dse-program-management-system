  ...
  return (
    <div className="mt-5 text-[9px] leading-[1.15]">
      <p className="mb-1 text-[10px]">
        * Levels in Learning Domain: Knowledge (Cognitive-C), Attitude
      ...
      <div className="grid grid-cols-3 border border-black">
        {Object.entries(TAXONOMY).map(([domain, rows], domainIndex) => (
          <div
            key={domain}
            className={domainIndex < 2 ? "border-r border-black p-2" : "p-2"}
          >
            <p className="mb-1 text-[10px] font-bold">{domain}</p>
            <table className="w-full border-collapse text-[9px] leading-[1.1]">
              <tbody>
                {rows.map(([number, label, code]) => (
                  <tr key={code}>
                    <td className="w-[12%] border border-black px-1 py-[2px] text-center">
                      {number}
                    </td>
                    <td className="border border-black px-1.5 py-[2px]">
                      {label}
                    </td>
                    <td className="w-[13%] border border-black px-1 py-[2px] text-center">
                      {code}
                    </td>
                  </tr>
                ))}
      ...
      <div className="grid h-[calc(100vh-250px)] min-h-[650px] gap-4 lg:grid-cols-[210px_minmax(0,1fr)]">
        ...
        <main ref={viewerRef} className="relative min-h-0 overflow-auto rounded-lg border bg-muted/40">
          ...
          <div
            ref={printRootRef}
            className="mx-auto"
            style={{ width: PAGE_WIDTH * zoom + VIEWER_PADDING * 2, padding: VIEWER_PADDING }}
          >
                    ...
                    <thead>
                      <tr>
                        ...
                        <TH colSpan={3} className="bg-[#E2EEDB] text-center font-normal">
                          Levels in Learning Domain:<br />Knowledge (Cognitive-C), Attitude (Affective-A), Skills (Psychomotor-P)
                        </TH>
                      </tr>
                      <tr>
            ...
            {weeklyPages.map((weeks, index) => (
              <Page zoom={zoom} pageNumber={weeklyStartPage + index} key={`lesson-${index}`}>
                <div id={index === 0 ? "lesson-plan" : undefined} className="h-full px-[54px] py-[42px]">
                  ...
                  <Table>
                    <colgroup><col className="w-[5%]" /><col className="w-[9%]" /><col className="w-[15%]" /><col className="w-[8%]" /><col className="w-[20%]" /><col className="w-[18%]" /><col className="w-[15%]" /><col className="w-[10%]" /></colgroup>
                    <thead><tr><TH>Week</TH><TH>Hour (L/T/P/O)</TH><TH>Topic</TH><TH>CLO</TH><TH>Lesson Learning Outcomes</TH><TH>Teaching Method / Activity</TH><TH>Assessment</TH><TH>Resources</TH></tr></thead>
            ...
            <Page zoom={zoom} pageNumber={responsibilityPage}>
              <div id="responsibility" className="h-full px-[54px] py-[42px]">
                ...
                {document.responsibilities.length === 0 ? <p className="text-[11px]">No student responsibilities have been recorded.</p> : <ul className="list-disc space-y-1.5 pl-5 text-[11px] leading-[1.4]">{document.responsibilities.map((item, index) => <li key={index}>{item}</li>)}</ul>}
            ...
            <Page zoom={zoom} pageNumber={policyPage}>
              <div id="policy" className="h-full px-[54px] py-[42px]">
                ...
                <div className="space-y-3 text-[11px] leading-[1.4]">{[["Attendance & Preparation", document.policy.attendancePreparation], ["Academic Integrity", document.policy.academicIntegrity], ["Homework & Assignments", document.policy.assignmentsLateSubmission], ["Examinations", document.policy.examinationRules], ["Penalties", document.policy.penaltiesConsequences]].map(([label, value]) => (<div key={label}><p className="font-bold">{label}</p><p className="mt-0.5 whitespace-pre-wrap">{displayValue(value)}</p></div>))}</div>
