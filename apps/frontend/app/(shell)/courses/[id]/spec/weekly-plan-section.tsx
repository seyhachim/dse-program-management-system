...
export function WeeklyPlanSectionForm({
  ...
  const remove = async (id: string) => {
    ...
    const ok = await onPersist(value.filter((item) => item.id !== id));
    if (ok) {
      setExpandedWeekIds((current) => {
        const next = new Set(current);
        next.delete(id);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
          ...
          <div className="space-y-2.5">
            {value.map((week) => {
              ...
              const teaching = week.teachingMethodIds.map(
                (methodId) =>
                  teachingMethodById.get(methodId)?.name ?? "Unknown method",
              );
              ...
              return (
                ...
                >
                  <div className="flex items-stretch">
                    ...
                    >
                      <div className="grid min-w-0 gap-3 sm:grid-cols-[52px_minmax(0,2fr)_minmax(120px,1fr)_76px_minmax(110px,1fr)_110px_22px] sm:items-center">
                          ...
                          <div className="mt-1.5 flex flex-wrap gap-1">
                            {week.cloCodes.length > 0 ? (
                              week.cloCodes.map((code) => (
                                <span
                                  key={code}
                                  className={`inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-medium ${cloChip(code)}`}
                                >
                                  {code}
                                </span>
                              ))
                        ...
                        <SummaryValue
                          label="Teaching"
                          value={summarizeItems(teaching, "No method")}
                        />
                        <SummaryValue
                          label="SLT"
                          value={`${weekSltForm(week)} h`}
                          strong
                        />
                        <SummaryValue
                          label="Assessment"
                          value={summarizeItems(assessments, "None")}
                        />

                        <div>
                          <WeekStatus attention={attention} />
                        </div>
                  ...
                  {expanded ? (
                    <div
                      id={`week-details-${week.id}`}
                      className="border-t border-border bg-muted/10 p-3 sm:p-4"
                    >
                      <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">
                        <DetailCard title="Topic & Outcomes">
                          <DetailList
                        ...
                        <DetailCard title="Alignment & Validation">
                        ...
                        <DetailCard title="Needs Attention">
                          {attention.length === 0 ? (
                            <div className="flex items-center gap-2 text-sm text-emerald-700 dark:text-emerald-300">
                              ...
                              Planning information is complete.
                            </div>
                          ) : (
                            <ul className="space-y-2 text-sm">
                              {attention.map((issue) => (
                                <li key={issue} className="flex items-start gap-2">
                                  ...
                                  <span>{attentionLabel(issue)}</span>
                                </li>
                              ))}
                            </ul>
                          )}
                      ...
                      <div className="mt-4 flex flex-col gap-2 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-xs text-muted-foreground">
                          Edit opens the existing weekly-plan form. Saving continues through the same Course Specification persistence flow.
                        </p>
                        <Button size="sm" onClick={() => openEdit(week.id)}>
        ...
        <div className="flex items-start gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          ...
          <span>
            SLT = Contact Hours (Lecture + Tutorial + Practice + Other) + Independent Learning Hours (NF2F). Expand a week to review the full breakdown, alignment and attention items.
          </span>
        </div>
      </div>
...
function WeekStatus({ attention }: { attention: string[] }) {
  if (attention.length === 0) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-1 text-[11px] font-medium text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-950/40 dark:text-emerald-300">
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2 py-1 text-[11px] font-medium text-amber-700 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-300">
      ...
      {items.length > 0 ? (
        <div className="flex flex-wrap gap-1.5">
          {items.map((item, index) => (
            <span
              key={`${item}-${index}`}
              className="inline-flex rounded-md bg-muted px-2 py-1 text-xs text-foreground"
            >
              {item}
            </span>
          ))}
    ...
    <div className="space-y-3">
      <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard
          ...
          label="CLOs Covered"
          value={coverage.length > 0 ? `${coveredClos} / ${coverage.length}` : "—"}
          detail={
            coverage.length > 0
      ...
      <div className="grid items-start gap-3 lg:grid-cols-[minmax(0,1.65fr)_minmax(260px,0.75fr)]">
    ...
    <Card>
      ...
      {coverage.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border px-4 py-8 text-center">
          <p className="text-sm text-muted-foreground">Link CLOs to weeks to see coverage.</p>
        </div>
      ) : (
        <>
          <div className="hidden grid-cols-[52px_minmax(0,1fr)_105px_60px] items-center gap-2 border-b border-border pb-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground sm:grid">
            <span>CLO</span>
            <span>Coverage</span>
            <span>Weeks</span>
          ...
          <div>
            {coverage.map((item) => {
              ...
              const width = Math.min(100, (hours / maxAllocation) * 100);
              const weeks = plan
              ...
              return (
                <div
                  key={item.code}
                  className="grid gap-2 border-b border-border/60 py-3 last:border-b-0 sm:grid-cols-[52px_minmax(0,1fr)_105px_60px] sm:items-center"
                >
                  <span
                    className={`inline-flex w-fit rounded-md px-1.5 py-0.5 text-xs font-semibold ${cloChip(item.code)}`}
                  >
                    {item.code}
                  </span>
                  <div className="min-w-0">
                    <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                      <div
                        ...
                        style={{ width: `${width}%`, backgroundColor: cloColor(item.code) }}
                      />
                    </div>
                    <p className="mt-1 text-[10px] text-muted-foreground">
                      {item.weeks > 0
                        ? `${item.weeks} ${item.weeks === 1 ? "week" : "weeks"} linked`
                        : "Not linked"}
                    </p>
                  </div>
          ...
          <div className="mt-2 flex items-center justify-between border-t border-border pt-3">
            <span className="text-xs font-semibold text-foreground">Total Student Learning Time</span>
            <span className="text-sm font-bold text-foreground">{totalSlt.toFixed(1)} h</span>
          </div>
        </>
      )}
    </Card>
    ...
    <Card>
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">Missing planning information.</p>
        </div>
        {attentionWeeks > 0 ? (
      ...
      <div className="mt-3 rounded-lg border border-blue-200/70 bg-blue-50/50 p-2.5 dark:border-blue-900/40 dark:bg-blue-950/20">
        <div className="flex items-start gap-2">
          ...
          <p className="text-[10px] leading-4 text-muted-foreground">
            Attention means planning information is incomplete. It does not indicate poor teaching quality or non-compliance.
          </p>
        </div>
      </div>
    </Card>
...
function weekLessonOutcomes(week: WeeklyPlanForm[number]): string[] {
...
function weekActivities(week: WeeklyPlanForm[number]): string[] {
