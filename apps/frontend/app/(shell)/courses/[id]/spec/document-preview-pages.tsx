...
export function DocumentPages({ document, zoom }: { document: CourseDocumentModel; zoom: number }) {
  ...
  return (
    <>
      ...
      <Page zoom={zoom} pageNumber={ratingScalePage}><div id="rating-scale" className="h-full px-[54px] py-[42px]"><SectionTitle number="24">Rating Scale</SectionTitle>{document.gradingScale?.grades.length ? <Table><colgroup><col className="w-[25%]" /><col className="w-[25%]" /><col className="w-[25%]" /><col className="w-[25%]" /></colgroup><thead><tr><TH>Letter Grade</TH><TH>Grade Point</TH><TH>Score</TH><TH>Explanation</TH></tr></thead><tbody>{document.gradingScale.grades.map((grade) => <tr key={grade.id}><TD className="text-center">{grade.letterGrade}</TD><TD className="text-center">{grade.gradePoint.toFixed(2)}</TD><TD className="text-center">{grade.scoreLabel}</TD><TD>{grade.explanation}</TD></tr>)}</tbody></Table> : <p className="text-[11px]">No approved programme grading scale is bound to this Course Specification.</p>}<PageFooter courseCode={info.courseCode} page={ratingScalePage} /></div></Page>
