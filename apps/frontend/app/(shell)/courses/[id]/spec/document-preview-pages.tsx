...
export function DocumentPages({ document, zoom }: { document: CourseDocumentModel; zoom: number }) {
  ...
  const projectBased = terminology.projectBased;
  const mappingMethodLabel = projectBased
    ? "Supervision / Learning Method"
    : "Teaching Method";
  const mappingTitle = projectBased
    ? "Mapping of the Course Learning Outcomes to the Programme Learning Outcomes, Supervision / Learning Methods and Assessment Methods"
    : "Mapping of the Course Learning Outcomes to the Programme Learning Outcomes, Teaching Methods and Assessment Methods";
  const totalContentSltLabel = projectBased
    ? "Total SLT for Final Project Work"
    : "Total SLT for Course Content";
  const resourcesTitle = projectBased
    ? "Required Resources for Final Project"
    : "Required Resources to Deliver the Course";
  const policyTitle = projectBased ? "Project Policy" : "Course Policy";
  ...
  return (
    <>
      ...
      <Page zoom={zoom} pageNumber={5}><div id="mapping" className="h-full px-[54px] py-[42px]"><PartTwoRow>
        <SectionTitle number="15">{mappingTitle}</SectionTitle>
      ...
      {weeklyPages.map((weeks, index) => <Page zoom={zoom} pageNumber={weeklyStartPage + index} key={`lesson-${index}`}><div id={index === 0 ? "lesson-plan" : undefined} className="h-full px-[54px] py-[42px]"><PartTwoRow><SectionTitle number="18">{terminology.detailedPlan}{weeklyPages.length > 1 ? ` — ${terminology.weekPlural} ${weeks[0]?.week ?? ""}–${weeks[weeks.length - 1]?.week ?? ""}` : ""}</SectionTitle><Table><colgroup><col className="w-[5%]" /><col className="w-[9%]" /><col className="w-[15%]" /><col className="w-[8%]" /><col className="w-[20%]" /><col className="w-[18%]" /><col className="w-[15%]" /><col className="w-[10%]" /></colgroup><thead><tr><TH>{terminology.weekSingular}</TH><TH>Hour (L/T/P/O)</TH><TH>{terminology.topic}</TH><TH>CLO</TH><TH>{terminology.learningOutcomes}</TH><TH>{terminology.teachingMethods}</TH><TH>{terminology.assessment}</TH><TH>{terminology.resources}</TH></tr></thead><tbody>{weeks.map((week) => <tr key={week.id}><TD>{week.week}</TD><TD>{[week.lectureHours, week.tutorialHours, week.practiceHours, week.otherHours].map((h) => h || "0").join("/")}</TD><TD>{week.topic}</TD><TD>{joinValues(week.cloCodes)}</TD><TD>{week.lloItems.length ? week.lloItems.map((item, i) => <div key={i}>{projectBased ? `Expected ${i + 1}` : `LLO${i + 1}`}: {item}</div>) : "—"}</TD><TD>{joinValues(week.teachingMethods.length ? week.teachingMethods : week.learningActivities)}</TD><TD>{joinValues([week.assessment, ...week.assessmentMethods].filter(Boolean))}</TD><TD>{joinValues(week.resources)}</TD></tr>)}</tbody></Table><div className="mt-4 grid grid-cols-3 gap-3 text-[9px]"><div className="rounded border border-black p-2"><strong>{terminology.learningActivities}</strong><p className="mt-1">{joinValues(weeks.flatMap((w) => w.learningActivities))}</p></div><div className="rounded border border-black p-2"><strong>{projectBased ? "Project Learning Strategies" : "Active Learning Strategies"}</strong><p className="mt-1">{joinValues(weeks.flatMap((w) => w.activeLearningStrategies))}</p></div><div className="rounded border border-black p-2"><strong>{terminology.resources}</strong><p className="mt-1">{joinValues(weeks.flatMap((w) => w.resources))}</p></div></div></PartTwoRow><PageFooter courseCode={info.courseCode} page={weeklyStartPage + index} /></div></Page>)}
      <Page zoom={zoom} pageNumber={resourcesPage}><div id="resources" className="h-full px-[54px] py-[42px]"><PartTwoRow><SectionTitle number="19">{resourcesTitle}</SectionTitle>{document.resources.length === 0 ? <p className="text-[11px]">No required resources have been confirmed.</p> : <Table><colgroup><col className="w-[18%]" /><col className="w-[27%]" /><col className="w-[25%]" /><col className="w-[30%]" /></colgroup><thead><tr><TH>Resource Type</TH><TH>Resource Name / Description</TH><TH>Link</TH><TH>Notes</TH></tr></thead><tbody>{document.resources.map((resource) => <tr key={resource.id}><TD>{displayDocumentValue(resource.resourceType)}</TD><TD>{displayDocumentValue(resource.title)}</TD><TD>{displayDocumentValue(resource.url)}</TD><TD>{displayDocumentValue(resource.notes)}</TD></tr>)}</tbody></Table>}</PartTwoRow><PageFooter courseCode={info.courseCode} page={resourcesPage} /></div></Page>
