...
import { ExternalLink, BookOpen, Plus, Save, Trash2 } from "lucide-react";
...
const CATEGORY_OPTIONS: {
  value: ResourceReferenceCategory;
  label: string;
}[] = [
  { value: "required", label: "Required Reference / Textbook" },
  { value: "recommended", label: "Recommended Textbook / Reading" },
  { value: "other", label: "Other Teaching & Learning Material" },
];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-lg font-bold text-foreground">
            References, Textbooks &amp; Learning Materials
          </h2>
          <p className="text-sm text-muted-foreground">
            Maintain the structured records used by Course Specification §20.
            Required delivery infrastructure such as computer labs and software
            belongs in Teaching &amp; Learning §19.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <div>
            <p className="text-sm font-semibold text-foreground">
              Suggested Materials from Weekly Plan
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Add a weekly resource here only when it should also appear as a
              course-level §20 teaching and learning material.
            </p>
          </div>
            ...
            {suggestions.map((suggestion) => {
              ...
              return (
                <Button
                  ...
                  onClick={() =>
                    addResource(suggestion.resourceType, suggestion.weekIds)
                  }
                >
                  {alreadyAdded ? (
                    "Added"
      ...
      {draft.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-card px-6 py-12 text-center">
          ...
          <p className="mt-3 text-sm font-medium text-foreground">
            No §20 references or learning materials yet
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Add required references, recommended readings, or other course-level
            ...
            >
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {CATEGORY_OPTIONS.find(
                      (option) => option.value === item.referenceCategory,
                    )?.label ?? "Other Teaching & Learning Material"}
                  </p>
                </div>
                <Button
                  type="button"
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <label className="space-y-1 md:col-span-2">
                  ...
                  <select
                    value={item.referenceCategory}
                    onChange={(event) =>
                      updateItem(item.id, {
                        referenceCategory: event.target
                          .value as ResourceReferenceCategory,
                      })
                    }
                ...
                <label className="space-y-1 md:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="e.g. An Introduction to Statistical Learning: with Applications in Python"
                    className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                  />
                </label>
                  ...
                  <>
                    <label className="space-y-1 md:col-span-2">
                      ...
                      <input
                        ...
                        placeholder="e.g. Gareth James, Daniela Witten, Trevor Hastie, Robert Tibshirani"
                        className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                      />
                    </label>
                    <label className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateItem(item.id, { publisher: event.target.value })
                        }
                    ...
                    <label className="space-y-1">
                      ...
                      <input
                        ...
                        placeholder="e.g. 2023 (2nd Edition)"
                        className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                      />
                    </label>
                    <label className="space-y-1 md:col-span-2">
                      ...
                      <input
                        value={item.isbn}
                        onChange={(event) =>
                          updateItem(item.id, { isbn: event.target.value })
                        }
                        placeholder="e.g. 978-3031387467"
                        className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                      />
                    </label>
                  </>
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateItem(item.id, { resourceType: event.target.value })
                    }
                    placeholder="e.g. Dataset, Notebook, Slides, Reading"
                    className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                  />
                </label>
                ...
                <label className="space-y-1 md:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Edition notes, access requirements, description, or other information"
              ...
              {weeklyPlan.length > 0 ? (
                <div className="mt-4 rounded-lg border border-border bg-muted/20 p-3">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Optional provenance showing which teaching weeks use this
                    material.
                  </p>
                  <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {weeklyPlan.map((week) => {
                      const checked = item.evidenceWeekIds.includes(week.id);
                      const planned = week.teachingResourceTypes.includes(
                        item.resourceType,
                      );
                      return (
                        <label
                          key={week.id}
                          className={`flex cursor-pointer items-start gap-2 rounded-md border p-2 text-xs ${planned ? "border-primary/40 bg-primary/5" : "border-border"}`}
                        >
                          <input
                            type="checkbox"
                            checked={checked}
                            onChange={() => toggleEvidenceWeek(item.id, week.id)}
                            className="mt-0.5"
                          />
                          <span>
                            <strong>Week {week.week}</strong> — {week.topic || "Untitled week"}
                          </span>
                        </label>
                      );
                    })}
      ...
      <div className="rounded-lg border border-blue-200/70 bg-blue-50/50 px-3 py-2 text-xs text-muted-foreground dark:border-blue-900/40 dark:bg-blue-950/20">
        §19 delivery infrastructure is defined in Teaching &amp; Learning. This
        Resources workspace owns the detailed references, textbooks, readings,
        links, and teaching materials used by §20.
      </div>
    </div>
  );
}
