...
describe("Course Information inline synopsis editing", () => {
  ...
  test("keeps the latest Course Information authoring shell while wiring inline save", async () => {
    ...
    expect(source).toContain("<CourseInfoSection");
    expect(source).not.toContain("<Dialog");
  });

  test("read-only history view uses the inline Overview contract without exposing editing", async () => {
  ...
  test("Course Information persists synopsis only and renders prerequisites read-only", async () => {
