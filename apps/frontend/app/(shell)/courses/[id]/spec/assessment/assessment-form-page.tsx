            ...
            <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-end gap-2 border-t border-border pt-4">
                  <Button variant="outline" nativeButton={false} render={<Link href={backHref}>Cancel</Link>} />
                  <Button onClick={submit} disabled={saving}>
                    {saving ? "Saving…" : "Save Assessment"}
                  </Button>
                </div>
              </div>
              <AssessmentSummary draft={draft} rubrics={rubrics} items={items} assessmentId={assessmentId} />
            </div>
          ) : null}
        </div>
      </main>
...
function AssessmentSummary({ draft, rubrics, items, assessmentId }: { draft: AssessmentForm; rubrics: Rubric[]; items: AssessmentForm[]; assessmentId: string | null }) {
  ...
  const displayWeight = (value: number) => Number.isInteger(value) ? String(value) : value.toFixed(1);
  const otherAssessments = items.filter((item) => item.id !== assessmentId);
  const allocatedWeight = otherAssessments.reduce((total, item) => total + numberWeight(item.weight), 0);
  ...
  return <aside className="space-y-4 xl:sticky xl:top-6">
    <div className="rounded-xl border border-border bg-card p-4">
      <h3 className="text-xs font-bold uppercase tracking-wide text-accent-foreground">Assessment Summary</h3>
    ...
    <div className="rounded-xl border border-border bg-card p-4">
      <h3 className="text-xs font-bold uppercase tracking-wide text-accent-foreground">Assessment Plan Overview</h3>
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-lg border border-border bg-muted/20 px-3 py-2"><p className="text-[11px] text-muted-foreground">Current allocated</p><p className="mt-1 text-lg font-semibold text-foreground">{displayWeight(allocatedWeight)}%</p></div>
        <div className="rounded-lg border border-border bg-muted/20 px-3 py-2"><p className="text-[11px] text-muted-foreground">Remaining</p><p className="mt-1 text-lg font-semibold text-foreground">{displayWeight(remainingWeight)}%</p></div>
      </div>
      <div className="mt-4 overflow-hidden rounded-lg border border-border">
        {otherAssessments.length ? otherAssessments.map((item) => <div key={item.id} className="flex justify-between gap-3 border-b border-border px-3 py-2 text-xs"><span className="truncate">{item.name || "Untitled assessment"}</span><span className="shrink-0 font-semibold">{item.weight !== "" ? `${displayWeight(numberWeight(item.weight))}%` : "—"}</span></div>) : <p className="px-3 py-3 text-xs text-muted-foreground">No other assessments yet.</p>}
        <div className="flex justify-between gap-3 bg-accent/5 px-3 py-2 text-xs"><span><span className="block font-semibold">{draft.name || "This assessment"}</span><span className="text-muted-foreground">(This assessment)</span></span><span className="shrink-0 font-semibold text-accent-foreground">{draft.weight !== "" ? `${displayWeight(currentWeight)}%` : "—"}</span></div>
      </div>
      <div className="mt-4 space-y-2 border-t border-border pt-3">
        <SummaryRow label="Other assessments" value={`${displayWeight(allocatedWeight)}%`} />
        <SummaryRow label="This assessment" value={draft.weight !== "" ? `${displayWeight(currentWeight)}%` : "—"} />
        <SummaryRow label="Total after save" value={`${displayWeight(totalAfterSave)}%${Math.abs(totalAfterSave - 100) < 0.001 ? " ✓" : ""}`} />
      </div>
      <p className={`mt-3 rounded-lg px-3 py-2 text-xs ${totalAfterSave > 100 ? "border border-status-live/40 bg-status-live/10 text-status-live" : "border border-accent/30 bg-accent/5 text-muted-foreground"}`}>{totalAfterSave > 100 ? "Total assessment weight would exceed 100%." : "The total weight of all assessments should equal 100%."}</p>
    </div>

    <div className="rounded-xl border border-border bg-card p-4">
      <h3 className="text-xs font-bold uppercase tracking-wide text-accent-foreground">Checklist</h3>
      <div className="mt-3 space-y-2">{checks.map(([label, ok]) => <div key={label} className="flex items-center gap-2 text-sm"><span aria-hidden>{ok ? "✓" : "○"}</span><span className={ok ? "text-foreground" : "text-muted-foreground"}>{label}</span></div>)}</div>
      <p className="mt-3 text-xs text-muted-foreground">Due week, duration, and feedback timeline are optional. Saving still preserves drafts.</p>
    </div>

    <div className="rounded-xl border border-border bg-card p-4">
      <h3 className="text-xs font-bold uppercase tracking-wide text-accent-foreground">Weight Guidance</h3>
      <p className="mt-3 text-xs leading-5 text-muted-foreground">Consider the importance, scope, and learning depth of this assessment relative to the other assessments. The lecturer remains responsible for the final weighting decision.</p>
    </div>
  </aside>;
}

function SummaryRow({ label, value }: { label: string; value: string }) {
  return <div className="grid grid-cols-[92px_minmax(0,1fr)] gap-3 border-b border-border pb-2 last:border-b-0 last:pb-0"><dt className="text-xs text-muted-foreground">{label}</dt><dd className="break-words text-xs font-semibold text-foreground">{value}</dd></div>;
}
