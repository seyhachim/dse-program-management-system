...
export type CourseDocumentModel = {
  ...
  totals: {
    ...
    continuousAssessmentSlt: number | null;
    finalAssessmentSlt: number | null;
...
function assessmentSlt(values: {
  physicalHours: string;
  onlineHours: string;
  independentHours: string;
}): number | null {
  const physical = numericOrNull(values.physicalHours);
  const online = numericOrNull(values.onlineHours);
  const independent = numericOrNull(values.independentHours);
  if (physical == null && online == null && independent == null) return null;
  return (physical ?? 0) + (online ?? 0) + (independent ?? 0);
}
...
export function buildCourseDocument({
  ...
  const documentWeeks = weeklyPlan.map((week) => {
    ...
    return {
      ...
      lloItems,
      learningActivities: learningActivities.length ? learningActivities : week.activities,
  ...
  const documentAssessments = assessments
    ...
    .map((assessment) => {
      const rubric = rubricById.get(assessment.rubric);
      return {
        id: assessment.id,
        ...
        physicalHours: assessment.physicalHours,
        onlineHours: assessment.onlineHours,
        independentHours: assessment.independentHours,
        totalSltHours: assessmentSlt(assessment),
        ...
        evaluationDefinition: rubric?.description ?? "",
        rubricName: rubric?.name ?? "",
        rubricScoring: rubric ? rubricScaleSummary(rubric.levels) : "",
  ...
  const documentMapping = activeClos.map((clo) => {
    const focusPercent = cloFocusPercent(clo.sltHours ? Number(clo.sltHours) : null, courseTotalSlt);
    return {
      cloCode: clo.code,
  ...
  const courseContentSlt = documentWeeks.reduce((sum, week) => sum + (Number(week.sltHours) || 0), 0);
  const assessmentWeight = documentAssessments.reduce((sum, assessment) => sum + (Number(assessment.weight) || 0), 0);
  const continuousAssessmentSlt = sumKnown(
    documentAssessments
      .filter((assessment) => assessment.assessmentCategory === "continuous")
      .map((assessment) => assessment.totalSltHours),
  );
  const finalAssessmentSlt = sumKnown(
    documentAssessments
      .filter((assessment) => assessment.assessmentCategory === "final")
      .map((assessment) => assessment.totalSltHours),
  );
  const assessmentSlt = sumKnown(documentAssessments.map((assessment) => assessment.totalSltHours));
  const calculatedGrand = assessmentSlt == null ? null : courseContentSlt + assessmentSlt;
  ...
  return {
    ...
    responsibilities: (responsibility?.items ?? []).map((item) => item.text.trim()).filter(Boolean),
    ...
    totals: {
      ...
      finalAssessmentSlt,
      assessmentSlt,
      grandSlt: courseTotalSlt ?? calculatedGrand,
      assessmentWeight,
    },
  };
}
