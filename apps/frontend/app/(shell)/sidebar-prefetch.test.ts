...
describe("sidebar route prefetch wiring", () => {
  test("prefetches route and protected data only after authenticated intent", () => {
    expect(source).toContain("if (!me || matchesRoute(pathname, routePath)) return");
  ...
  test("does not re-prefetch the route that is already active", () => {
    expect(source).toContain("matchesRoute(pathname, routePath)");
  });
