  ...
  if (!today) {
    return (
      <div className="space-y-5">
        ...
        <Link href="/telegram/schedule" className="block min-h-11 rounded-xl bg-slate-950 px-4 py-3 text-center text-sm font-semibold text-white">
            ...
            <div className="border-l-4 border-blue-500 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">{primary.courseCode} · {primary.sectionCode}</p>
                  <h3 className="mt-1 text-xl font-semibold leading-tight text-slate-950">{primary.courseTitle}</h3>
                </div>
              ...
              {showArrivalAction ? (
                <Link
                  href={`/telegram/classes/${encodeURIComponent(primary.offeringId)}/delivery`}
                  className="mt-4 flex min-h-12 w-full items-center justify-center rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white shadow-sm active:bg-blue-700"
                >
                  {primary.arrivalStatus === "NotYet" ? "Lecturer has arrived" : "Confirm lecturer arrival"}
                </Link>
              ) : null}

              {primary.canConfirmLecturerArrival ? (
                <p className="mt-3 text-xs leading-5 text-slate-500">
                  Class Monitor / Sub-class Monitor access is verified by DSE PMS.
                </p>
              ) : null}
            </div>
          </article>
            ...
            {later.map((item) => (
              <Link key={item.meetingId} href={`/telegram/classes/${encodeURIComponent(item.offeringId)}`} className="flex min-h-14 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3">
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-semibold text-slate-900">{item.courseTitle}</span>
                  <span className="block truncate text-xs text-slate-500">{item.room || "Room TBA"} · {item.activityType}</span>
                </span>
